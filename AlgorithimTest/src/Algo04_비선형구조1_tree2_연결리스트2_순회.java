...
public class Algo04_비선형구조1_tree2_연결리스트2_순회 {
	static class Node{
		int data; //노드값
	...
	public static void order(Node node) {
		if(node!=null) {
//			System.out.println(node.data+" "); //전위: 0 1 3 4 2 5
			if(node.left !=null) {
				order(node.left);
			}
//			System.out.println(node.data+" "); //중위: 3 1 4 0 2 5
			if(node.right !=null) {
				order(node.right);
			}
//			System.out.println(node.data+" "); //후위: 3 4 1 5 2 0
		}
	}//end order
